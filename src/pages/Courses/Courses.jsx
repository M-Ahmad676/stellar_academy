...
export default function Courses() {
  ...
  return (
    <div className='overflow-hidden min-h-screen'>
      ...
      <div className='grid grid-cols-[1fr_1fr_1fr] grid-rows-3 justify-items-center gap-y-10 py-12'>
      {programsOffered.map((program,index) => (
        <div key={index} className='max-w-[20rem] w-full h-auto rounded-lg'>
          <div>
            <LazyLoadingImage title={program.title} path={program.image} styling="object-cover w-full rounded-lg"/>
          </div>
          <div className='h-[10rem] p-5 space-y-4'>
            <h4 className='font-medium text-orange-600 text-[1rem]'>{program.grade}</h4>
            <h2 className='text-[1.2rem]'>{program.title}</h2>
